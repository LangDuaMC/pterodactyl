...
export const rawDataToServerObject = ({ attributes: data }: FractalResponseData): Server => {
    const tenant = data.tenant && typeof data.tenant === 'object' ? data.tenant : null;

    return {
        id: data.identifier,
        identifier: data.server_identifier,
        internalId: data.internal_id,
        tenantId: data.tenant_id ?? tenant?.id ?? (typeof data.tenant === 'number' ? data.tenant : null),
        tenant: tenant
            ? {
                  id: tenant.id,
                  uuid: tenant.uuid,
                  name: tenant.name,
                  description: tenant.description && tenant.description.length > 0 ? tenant.description : null,
              }
            : null,
